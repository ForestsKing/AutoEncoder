...
if __name__ == "__main__":
    ...
    dfs = [
        pd.read_csv(path + filename, sep=";", index_col="datetime", parse_dates=True)
    ...
    for df in tqdm(dfs):
        ...
        model = AutoEncoder()
        model, scaler, threshold, avg_train_losses, avg_valid_losses = train(
            X_train=X_train, scaler=scaler, model=model, showloss=False, epochs=100
        )
        prediction = test(
            X_test=X_test, model=model, scaler=scaler, threshold=threshold
        )
